-- # Answer.GettingStarted.Ex0202
-- ## 練習問題 2.2
-- リストが、与えられた比較関数にしたがってソートしたものであることをチェックする`isSorted`関数を実装せよ。
--
{-# LANGUAGE GHC2024 #-}
module Answer.GettingStarted.Ex0202
    ( isSorted
    , isSorted'
    , isSortedBy
    , isSortedBySpec
...
isSorted gt = and . (zipWith cmp <*> drop 1)
    where
        cmp x y = not (gt x y)
--
...
--
isSorted' :: (a -> a -> Bool) -> [a] -> Bool
isSorted' cmp = and . (zipWith cmp <*> drop 1)
--
-- | isSortedBy
...
isSortedBy cmp = all (not . (GT ==)) . (zipWith cmp <*> drop 1)
--
-- | isSortedBySpec
